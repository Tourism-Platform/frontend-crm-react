...
import { emptyHopFormRow } from "@/entities/supplier/converters/flight";

import { FLIGHT_PRODUCT_NAME_FIELD, type TSlotProps } from "../../model";
...
const GeneralInfoBase: FC<TSlotProps> = ({
	...
	const { fields, append, remove } = useFieldArray({
		...
		name: `${ENUM_FORM_FLIGHT_SECTION.GENERAL}.${ENUM_FORM_FLIGHT_PRODUCT.HOPS}`
	});

	return (
		<div className="grid gap-4">
			<div className="grid gap-x-4 gap-y-1 grid-cols-2">
				{FLIGHT_PRODUCT_NAME_FIELD.map(({ key, ...item }) => (
					<CustomField
						key={key}
						control={form.control}
						name={`${ENUM_FORM_FLIGHT_SECTION.GENERAL}.${key}`}
						t={t}
						{...item}
					/>
				))}
