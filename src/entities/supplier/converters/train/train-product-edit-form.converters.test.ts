...
describe("mapTrainProductToEditForm", () => {
	it("maps general, fares and pricing sections", () => {
		const form = mapTrainProductToEditForm(PRODUCT);

		expect(form.general.name).toBe("Night Express");
	...
	it("maps empty product defaults", () => {
		const form = mapTrainProductToEditForm(null);
	...
	it("maps whole pricing charge", () => {
		const form = mapTrainProductToEditForm({
			...PRODUCT,
