...
const syncPerFareRows = (form: UseFormReturn<TFlightProductEditSchema>) => {
	if (
		form.getValues(
	...
	const faresList =
		form.getValues(
			`${ENUM_FORM_FLIGHT_SECTION.FARES}.${ENUM_FORM_FLIGHT_FARES.FARES_LIST}`
		) ?? [];
	const current =
		form.getValues(
			`${ENUM_FORM_FLIGHT_SECTION.PRICING}.${ENUM_FLIGHT_PRODUCT_PRICING_FIELD.FARES}`
		) ?? [];
