...
const BusinessSettingsBase: FC = () => {
	...
	} = useOptionalResourceQuery(useGetOperatorBusinessInfoQuery());
	const [updateBusiness, { isLoading: isUpdating }] =
		useUpdateOperatorBusinessInfoMutation();

	const form = useForm<TOperatorBusinessSchema>({
		resolver: zodResolver(OPERATOR_BUSINESS_SCHEMA),
		mode: "onSubmit"
	});
	...
	async function onSubmit(data: TOperatorBusinessSchema) {
		try {
