...
export const LibrarySupplementEditPage: FC = () => {
	...
	const tabs = useMemo(
		() =>
			SUPPLEMENT_EDIT_TABS_LIST.filter(
				(tab) => tab.type !== ENUM_SUPPLEMENT_EDIT_TAB.MEDIA
			),
		[]
	);
	...
	useEffect(() => {
		if (!isCreate && libraryEvent) {
			form.reset(libraryEvent as TSupplementEditSchema);
		}
	...
	const createSectionSubmit = async (section?: ENUM_FORM_SECTION_TYPE) => {
		...
		try {
			if (isCreate) {
				...
				navigate(
					buildRoute(ENUM_PATH.LIBRARY.EVENT_SUPPLEMENT, {
