...
import { INFORMATION_EDIT_TABS_LIST } from "@/widgets/tours/events/information-edit/model";

export const LibraryInfoEditPage: FC = () => {
	...
	const tabs = useMemo(
		() =>
			INFORMATION_EDIT_TABS_LIST.filter(
				(tab) => tab.type !== ENUM_INFORMATION_EDIT_TAB.MEDIA
			),
		[]
	);
	...
	useEffect(() => {
		if (!isCreate && libraryEvent) {
			form.reset(libraryEvent as TInfoEditSchema);
		}
	...
	const createSectionSubmit = async () => {
		...
		try {
			if (isCreate) {
				...
				navigate(
					buildRoute(ENUM_PATH.LIBRARY.EVENT_INFO, {
