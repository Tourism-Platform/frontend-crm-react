...
import {
	...
	alignHotelPerRoomExpenses
} from "@/entities/supplier";

import { PerRoomByClassCard } from "./per-room-by-class-card";
import { PerRoomCard } from "./per-room-card";

const syncPerRoomExpenses = (form: UseFormReturn<THotelProductEditSchema>) => {
	if (
		form.getValues(
			`${ENUM_FORM_HOTEL_SECTION.PRICING}.${ENUM_HOTEL_PRODUCT_PRICING_FIELD.PRICING_TYPE}`
	...
	const priceBasedOnClass = form.getValues(
		`${ENUM_FORM_HOTEL_SECTION.PRICING}.${ENUM_HOTEL_PRODUCT_PRICING_FIELD.PRICE_BASED_ON_CLASS}`
	);
	const expectedTyp = priceBasedOnClass
		? ENUM_HOTEL_PRODUCT_EXPENSE_TYP.PER_ROOM_CATEGORY
		: ENUM_HOTEL_PRODUCT_EXPENSE_TYP.PER_ROOM;
	const expenses = form.getValues(
		`${ENUM_FORM_HOTEL_SECTION.PRICING}.${ENUM_HOTEL_PRODUCT_PRICING_FIELD.EXPENSES}`
	);
	const roomsListLength =
		form.getValues(
			`${ENUM_FORM_HOTEL_SECTION.ROOMS}.${ENUM_FORM_HOTEL_PRODUCT_ROOMS.ROOMS_LIST}`
		)?.length ?? 0;

	if (
		expenses?.typ === expectedTyp &&
		expenses[ENUM_HOTEL_PRODUCT_PER_ROOM_EXPENSES_FIELD.ROOMS]?.length ===
			roomsListLength
	) {
		return;
	}
	...
	form.setValue(
		...
		alignHotelPerRoomExpenses({
			...
			current: expenses,
			...(form.getValues(
				`${ENUM_FORM_HOTEL_SECTION.PRICING}.${ENUM_HOTEL_PRODUCT_PRICING_FIELD.ADD_MARGIN_SEPARATELY}`
...
const PerRoomDetailsBase: FC<{
	...
	const pricingType = useWatch({
		control: form.control,
		name: `${ENUM_FORM_HOTEL_SECTION.PRICING}.${ENUM_HOTEL_PRODUCT_PRICING_FIELD.PRICING_TYPE}`
	});
	const priceBasedOnClass = useWatch({
		control: form.control,
		name: `${ENUM_FORM_HOTEL_SECTION.PRICING}.${ENUM_HOTEL_PRODUCT_PRICING_FIELD.PRICE_BASED_ON_CLASS}`
	});
	const addMarginSeparately = useWatch({
		control: form.control,
		name: `${ENUM_FORM_HOTEL_SECTION.PRICING}.${ENUM_HOTEL_PRODUCT_PRICING_FIELD.ADD_MARGIN_SEPARATELY}`
	});
