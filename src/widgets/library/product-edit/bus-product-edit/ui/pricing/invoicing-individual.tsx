...
import { PerVehicleDetails } from "./per-vehicle-details";
...
> = ({ form }) => {
	...
	const handleAddMarginSeparatelyChange = (checked: boolean) => {
		...
		form.setValue(
			`${ENUM_FORM_BUS_SECTION.PRICING}.${ENUM_BUS_PRODUCT_PRICING_FIELD.MARKUP}`,
				...
				<CustomOptionTabs
					...
					onValueChange={(val) =>
						form.setValue(
							...
							val as typeof ENUM_BUS_PRODUCT_PRICING_TYPE.FLAT_RATE
						)
					}
				>
						...
						>
							{tab.priceDetailsList ? (
								<div className="grid gap-4 mb-8">
												...
												<Fragment key={key}>
													...
													fieldIndex ===
														...
														<CustomInputSelect
															...
															name={`${ENUM_FORM_BUS_SECTION.PRICING}.${PRICING_MARKUP_FIELD.key}`}
															label={
													...
													<CustomField
														name={`${ENUM_FORM_BUS_SECTION.PRICING}.${key}`}
														control={form.control}
														t={t}
														{...item}
													/>
									...
									<FeeLinesField
										control={form.control}
										name={`${ENUM_FORM_BUS_SECTION.PRICING}.${ENUM_BUS_PRODUCT_PRICING_FIELD.FEES}`}
									/>
								</div>
							) : (
								<div className="mb-8">
									<PerVehicleDetails form={form} />
								</div>
							)}
						</CustomOptionTabsContent>
					))}
