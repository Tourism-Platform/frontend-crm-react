...
export const PerRoomByClassCard: FC<IPerRoomByClassCardProps> = ({
	...
	const { fields, append, remove } = useFieldArray({
		...
		name: `${roomsPath}.${ENUM_HOTEL_PRODUCT_PER_ROOM_EXPENSES_FIELD.CATEGORIES}`
	});
		...
		<Card>
			...
			<CardContent className="grid gap-4">
				{fields.map((field, categoryIndex) => {
					const categoryPath =
						`${roomsPath}.${ENUM_HOTEL_PRODUCT_PER_ROOM_EXPENSES_FIELD.CATEGORIES}.${categoryIndex}` as const;
					...
					return (
						<Fragment key={field.id}>
							<div className="grid gap-3">
								...
								<FeeLinesField
									control={form.control}
									name={`${categoryPath}.${ENUM_HOTEL_PRODUCT_CATEGORY_ROW_FIELD.FEES}`}
								/>
							</div>
