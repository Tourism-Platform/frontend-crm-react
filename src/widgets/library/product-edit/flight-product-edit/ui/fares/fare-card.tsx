...
export const FareCard: FC<IFareCardProps> = ({
	...
	const fareId = form.watch(
		`${ENUM_FORM_FLIGHT_SECTION.FARES}.${ENUM_FORM_FLIGHT_FARES.FARES_LIST}.${index}.${ENUM_FORM_FLIGHT_FARES.VARIANT_ID}`
	);
	const fareName = form.watch(
		`${ENUM_FORM_FLIGHT_SECTION.FARES}.${ENUM_FORM_FLIGHT_FARES.FARES_LIST}.${index}.${ENUM_FORM_FLIGHT_FARES.NAME}`
	);
				...
				<div className="grid grid-cols-2 gap-x-4 gap-y-1">
					{FARES_DATA_LIST().map(({ key, ...item }) => (
						<CustomField
							...
							name={`${ENUM_FORM_FLIGHT_SECTION.FARES}.${ENUM_FORM_FLIGHT_FARES.FARES_LIST}.${index}.${key}`}
							t={t}
							{...item}
						/>
					))}
