...
export const TOUR_CREATE_SCHEMA = z
	.object({
		...
		[ENUM_FORM.AGENCY_ID]: z.string().optional().nullable(),
		...
		[ENUM_FORM.DURATION]: z
			.object({
				from: z
					.number({
						message: msg(
							"create.form.errors.duration.from.required"
						)
					})
			...
			.superRefine((val, ctx) => {
				...
				if (nights < days - 1 || nights > days + 1) {
					ctx.addIssue({
						code: "custom",
						message: msg(
							"create.form.errors.duration.nightsMismatch"
						),
						path: ["to"]
					});
				}
			...
			.superRefine((val, ctx) => {
				...
				if (from && to && to < from) {
					ctx.addIssue({
						code: "custom",
						message: msg(
							"create.form.errors.ageRequires.toMismatch"
						),
						path: ["to"]
					});
				}
	...
	.superRefine((data, ctx) => {
		if (
			data[ENUM_FORM.TOUR_TYPE] === ENUM_TOUR_TYPES.PRIVATE &&
			!data[ENUM_FORM.AGENCY_ID]
		) {
			ctx.addIssue({
				code: "custom",
				message: msg("create.form.errors.agencyId.required"),
				path: [ENUM_FORM.AGENCY_ID]
			});
		}
	});
