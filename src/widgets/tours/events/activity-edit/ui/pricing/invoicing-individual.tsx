...
const InvoicingIndividualBase: FC<ISlotProps> = ({ form }) => {
	...
	const pricingType = useWatch({
		...
		name: `${ENUM_FORM_SECTION.PRICING}.${ENUM_ACTIVITY_PRICING_FIELD.PRICING_TYPE}`
	});
	const addMarginSeparately = useWatch({
		control: form.control,
		name: `${ENUM_FORM_SECTION.PRICING}.${ENUM_ACTIVITY_PRICING_FIELD.ADD_MARGIN_SEPARATELY}`
	});

	const handleAddMarginSeparatelyChange = (checked: boolean) => {
		form.setValue(
			`${ENUM_FORM_SECTION.PRICING}.${ENUM_ACTIVITY_PRICING_FIELD.ADD_MARGIN_SEPARATELY}`,
			checked
		);
		form.setValue(
			`${ENUM_FORM_SECTION.PRICING}.${ENUM_ACTIVITY_PRICING_FIELD.MARKUP}`,
	...
	return (
		<div className="grid gap-5">
			<div className="grid gap-1">
				...
				<CustomOptionTabs
					value={pricingType ?? ENUM_ACTIVITY_PRICING_TYPE.FLAT_RATE}
					onValueChange={(val) =>
						form.setValue(
