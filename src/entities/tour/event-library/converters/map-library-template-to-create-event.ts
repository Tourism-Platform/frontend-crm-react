...
/**
 * Library template (backend READ) → tour event create payload (contract 3.1).
 *
 * The library READ event is never sent back raw: its details cross the
 * READ → WRITE boundary (`mapEventDetailsReadToWrite`), which
 * - inline supply:  moves the read `details.spec` into `supply.inline.spec`;
 * - product supply: keeps only `{ source, product_id, scope }`
 *   (supplier/override/resolved spec are dropped).
 *
