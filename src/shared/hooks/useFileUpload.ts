...
export const useFileUpload = (
...
): [TFileUploadState, TFileUploadActions] => {
	const {
		maxFiles = Infinity,
		maxSize = Infinity,
		accept = "*",
	...
	const createPreview = useCallback(
		(file: File | TFileMetadata): string | undefined => {
			if (file instanceof File) {
	...
	const generateUniqueId = useCallback(
		(file: File | TFileMetadata): string => {
			if (file instanceof File) {
	...
	const addFiles = useCallback(
		(newFiles: FileList | File[]) => {
			...
			const validFiles: TFileWithPreview[] = [];
