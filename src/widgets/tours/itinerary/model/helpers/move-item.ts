...
export function moveItemInData(
    ...
    const isSameLocation = from.location === target.type;
    const isSameDay = from.location === "day" && target.type === "day" && from.day === target.day;
    const isSameTripDetails = from.location === "tripDetails" && target.type === "tripDetails";
    ...
    if (isSameContainer && from.nestedIndex === undefined && target.nestedIn === undefined) {
        // Moving within the same top-level array
        if (from.index < toIndex) {
            adjustedToIndex = toIndex - 1;
        }
    } else if (isSameContainer && from.nestedIndex === undefined && target.nestedIn !== undefined) {
