...
describe("Itinerary DnD Scenarios", () => {
    ...
    describe("1. Adding new elements (addItemToData)", () => {
        it("should add new item to an empty day", () => {
            const newItem = { id: "new", block_id: "new-block", event_type: ENUM_EVENT.TOUR_DETAILS, title: "New" } as any;
            const result = addItemToData(initialData, { type: "day", day: 2 }, 0, newItem, 1);
        ...
        it("should add new item directly into a group", () => {
            const newItem = { id: "new", block_id: "new-nested", event_type: ENUM_EVENT.ACTIVITY, title: "New Nested" } as any;
            // group-1 is at index 1 in Day 1
            const result = addItemToData(initialData, { type: "day", day: 1, nestedIn: 1 }, 1, newItem, 1);
            expect(result[1]?.days[1]?.[1]?.items).toHaveLength(2);
            expect(result[1]?.days[1]?.[1]?.items?.[1]?.block_id).toBe("new-nested");
        });
    });
    ...
    describe("3. Grouping and Ungrouping (moveItemInData)", () => {
        it("should pack a top-level item into a group", () => {
            ...
            expect(result[1]?.days[1]?.[0]?.items?.[1]?.block_id).toBe("day-1-item-1");
        });

        it("should extract a nested item to the top level", () => {
            const nestedItem = initialData[1].days[1][1].items[0];
            const result = moveItemInData(
                initialData,
                { optionId: 1, location: "day", day: 1, index: 1, nestedIndex: 0 },
                { type: "day", day: 1 },
                0,
                nestedItem,
                1
    ...
    describe("5. Deletion (removeItemFromData)", () => {
        it("should remove a top-level item", () => {
            const result = removeItemFromData(initialData, { optionId: 1, location: "day", day: 1, index: 0 });
            expect(result[1]?.days[1]).toHaveLength(1);
            expect(result[1]?.days[1]?.[0]?.block_id).toBe("group-1");
        });

        it("should remove a nested item", () => {
            const result = removeItemFromData(initialData, { optionId: 1, location: "day", day: 1, index: 1, nestedIndex: 0 });
